...
async function xemProfile(interaction) {
  ...
  if (!tt.initialized) {
    ...
    await bank.save(); // Lưu vĩnh viễn xuống Cloud Database
  ...
  const embedProfile = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        ...
        value: `\`${tt.luotTuLuyen ?? 20} / 20\` lượt`, // Dự phòng trường hợp lỗi bộ nhớ cache
        inline: true,
      },
    )
    .setDescription(
      `*Đường tu tiên dài đằng đẵng, vững bước kiên trì mới mong đắc đạo thành tiên!*`,
    )
    .setFooter({
      text: "Hãy bấm nút bên dưới để vận công hoặc tiến hành đột phá thiên kiếp • 30 phút hồi 1 lượt",
    });
