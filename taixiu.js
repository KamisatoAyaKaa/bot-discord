    ...
    handleTaiXiu: async function (interaction) {
        ...
        if (interaction.isChatInputCommand() && interaction.commandName === 'taixiu') {
            ...
            if (phienCuocHienTai && phienCuocHienTai.dangChay) {
                return interaction.reply({ content: '⚠️ Đang có một ván cược đang đếm ngược, hãy bấm chọn cửa ở bảng tin nhắn phía trên!', ephemeral: true });
            }
            ...
            const rowMain = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('tx_select_xiu').setLabel('Xỉu (3–10)').setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId('tx_select_tai').setLabel('Tài (11–18)').setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId('tx_select_chan').setLabel('Chẵn').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('tx_select_le').setLabel('Lẻ').setStyle(ButtonStyle.Danger)
            );

            const rowNums1 = new ActionRowBuilder().addComponents(
                [3, 4, 5, 6, 7].map(n => new ButtonBuilder().setCustomId(`tx_select_num_${n}`).setLabel(`Số ${n}`).setStyle(ButtonStyle.Secondary))
            );
            const rowNums2 = new ActionRowBuilder().addComponents(
                [8, 9, 10, 11, 12].map(n => new ButtonBuilder().setCustomId(`tx_select_num_${n}`).setLabel(`Số ${n}`).setStyle(ButtonStyle.Secondary))
            );
            const rowNums3 = new ActionRowBuilder().addComponents(
                [13, 14, 15, 16, 17].map(n => new ButtonBuilder().setCustomId(`tx_select_num_${n}`).setLabel(`Số ${n}`).setStyle(ButtonStyle.Secondary))
            );
            const rowNums4 = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('tx_select_num_18').setLabel('Số 18').setStyle(ButtonStyle.Secondary)
            );
            ...
            const countdownInterval = setInterval(async () => {
                ...
                if (phienCuocHienTai.thoiGianConLai <= 0) {
                    ...
                    for (const [idUid, dataCuoc] of Object.entries(phienCuocHienTai.nguoiChoi)) {
                        coNguoiChoi = true;
                        const pBank = bank.getPlayer(idUid);
                        let isWin = false;
                        let thuong = 0;

                        if (dataCuoc.cua === 'tai' && laTai) { isWin = true; thuong = dataCuoc.tien * 2; }
                        else if (dataCuoc.cua === 'xiu' && laXiu) { isWin = true; thuong = dataCuoc.tien * 2; }
                        else if (dataCuoc.cua === 'chan' && laChan) { isWin = true; thuong = dataCuoc.tien * 2; }
                        else if (dataCuoc.cua === 'le' && !laChan) { isWin = true; thuong = dataCuoc.tien * 2; }
                        else if (dataCuoc.cua.startsWith('num_') && parseInt(dataCuoc.cua.split('_')[1]) === tongDiem) { isWin = true; thuong = dataCuoc.tien * 11; }
                    ...
                    if (!coNguoiChoi) thongBaoKetQua += `*Không có ai tham gia đặt cược trong lượt này.*`;

                    await replyMessage.edit({
                        embeds: [new EmbedBuilder().setColor('#ff5555').setTitle('🛑 HẾT GIỜ ĐẶT CƯỢC!').setDescription(thongBaoKetQua)],
                        components: []
                    }).catch(() => { });
        ...
        // ==========================================
        if (interaction.isButton() && interaction.customId.startsWith('tx_select_')) {
            if (!phienCuocHienTai || !phienCuocHienTai.dangChay) {
                return interaction.reply({ content: '❌ Phiên cược đã kết thúc hoặc chưa bắt đầu!', ephemeral: true });
            }
            ...
            // FIX TẠI ĐÂY: Rút ngắn nhãn hiển thị xuống còn 34 ký tự (Dưới mức 45 quy định)
        ...
        if (interaction.isModalSubmit() && interaction.customId.startsWith('tx_modal_')) {
            if (!phienCuocHienTai || !phienCuocHienTai.dangChay) {
                return await interaction.reply({ content: '❌ Đã hết thời gian 30s cược, lệnh nhập tiền này không được tính!', ephemeral: true });
            }

            const cuaChon = interaction.customId.replace('tx_modal_', '');
            const tienCuocStr = interaction.fields.getTextInputValue('tx_input_money');
            const tienCuoc = parseInt(tienCuocStr);

            if (isNaN(tienCuoc) || tienCuoc < 50 || tienCuoc > 250000) {
                return await interaction.reply({ content: '❌ Số tiền cược không hợp lệ! Phải là số từ **$50** đến **$250,000**.', ephemeral: true });
            }

            if (player.balance < tienCuoc) {
                return await interaction.reply({ content: `❌ Bạn không đủ tiền! Ví hiện tại: **$${player.balance.toLocaleString()}**`, ephemeral: true });
            }

            // 1. Tiến hành trừ tiền cược trong ngân hàng chung ngay lập tức
            player.balance -= tienCuoc;

            // ĐIỂM SỬA QUYẾT ĐỊNH: Lưu lại số dư mới vào file database.json ngay khi vừa trừ tiền cược
            bank.save(); 
            ...
            // 2. FIX LỖI TƯƠNG TÁC: Thêm await để đảm bảo Discord đóng Form Modal thành công
