...
const gameTaiXiu = require('./taixiu.js'); 
const gameTuTien = require('./tutien/index.js'); // [BƯỚC MỚI] Import đầu mối Tu Tiên
...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        // [BƯỚC MỚI] Đăng ký lệnh Tu Tiên chính thức với Discord
        new SlashCommandBuilder()
            .setName('tutien')
            .setDescription('🧘 Bước vào con đường tu tiên nghịch thiên cải mệnh, thức tỉnh linh căn')
    ].map(command => command.toJSON());

    const rest = new REST({ version: '10' }).setToken(TOKEN);
    try {
        console.log('🔄 Đang đồng bộ hóa lệnh gạch chéo (/) lên hệ thống Discord...');
...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        if (interaction.isChatInputCommand()) {
            if (interaction.commandName === 'daily' || interaction.commandName === 'vi' || interaction.commandName === 'addtien') {
                await bank.handleBankCommands(interaction);
                return;
            }
        }

        // 2. Nhóm tương tác game Tài Xỉu
        if (
            (interaction.isChatInputCommand() && interaction.commandName === 'taixiu') || 
            (interaction.isButton() && interaction.customId.startsWith('tx_')) ||
            (interaction.isModalSubmit() && interaction.customId.startsWith('tx_modal_'))
        ) {
            await gameTaiXiu.handleTaiXiu(interaction);
            return;
        }

        // 3. [BƯỚC MỚI] Chuyển tiếp toàn bộ tương tác Tu Tiên sang folder tutien xử lý
        if (
            (interaction.isChatInputCommand() && interaction.commandName === 'tutien') ||
        ...
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: '❌ Có lỗi xảy ra khi xử lý lệnh này!', ephemeral: true }).catch(() => {});
        }
    }
});
